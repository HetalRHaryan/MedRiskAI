...
def dashboard():
    ...
    cursor.close()
    return render_template('dashboard.html', logs=json.dumps(logs, default=str))
...
def load_model(disease):
    ...
    print("Looking for:", os.path.join(FEATURES_DIR, f"{disease}_columns.json"))

    model_path = os.path.join(MODEL_DIR, f"{disease}.pickle")
    columns_path = os.path.join(FEATURES_DIR, f"{disease}_columns.json")
    ...
    # Some JSONs are lists, some are dicts
...
def one_hot_encode(form_data, feature_columns):
    """Transform user input into vector based on model features."""
    vector = [0] * len(feature_columns)

    for i, col in enumerate(feature_columns):
        if col in form_data:
            try:
                vector[i] = float(form_data[col])
            except ValueError:
                vector[i] = 0
        elif col.lower() in form_data:
            try:
                vector[i] = float(form_data[col.lower()])
            except ValueError:
                vector[i] = 0
        else:
            # Handle one-hot encoded categorical features like smoking_status
            if col in ['ever', 'former', 'never', 'not current']:
                if form_data.get('smoking_status') == col:
                    vector[i] = 1
...
def save_prediction(user_id, disease, input_data, result):
    ...
    cursor.execute(
        'INSERT INTO prediction_logs (user_id, disease, input_data, prediction_result) VALUES (%s, %s, %s, %s)',
        (user_id, disease, json.dumps(input_data, default=str), result)
    )
    mysql.connection.commit()
    cursor.close()
...
def predict(disease):
    if request.method == 'POST':
        ...
        print(form_data)
        try:
            model, feature_columns = load_model(disease)

            # Prepare input vector
            X = one_hot_encode(form_data, feature_columns)

            # Predict
            prediction = model.predict(X)
            result = str(prediction[0])  # convert to string for storage

            print(form_data)
            # Log result
            save_prediction(current_user.id, disease, form_data, result)

            flash(f'{disease.capitalize()} prediction: {result}', 'success')
            return redirect(url_for('history'))
...
def history():
    ...
    # Parse JSON for input_data (convert from string)
    for log in logs:
        try:
